...
export default async function CharactersPage({ params }: { params: Promise<{ workspaceId: string }> }) {
  ...
  let docs: Awaited<ReturnType<typeof getDocumentsByType>> = []
  try {
    docs = await getDocumentsByType(workspaceId, "character")
  } catch (error) {
    console.error('[CharactersPage] Error loading characters:', error)
  ...
  return (
    <div className="max-w-6xl mx-auto p-12 space-y-8">
      <div className="flex items-center justify-between">
        ...
        <CreateDocumentButton workspaceId={workspaceId} type="character" label="Tạo nhân vật" />
      </div>
      ...
      {docs.length === 0 ? (
        <div className="p-12 text-center rounded-[2rem] bg-surface-container-lowest border border-border/5 flex flex-col items-center space-y-4">
          <Users className="w-12 h-12 text-on-surface-variant/20" />
          <p className="text-on-surface-variant font-sans text-sm">Chưa có nhân vật nào được tạo.</p>
        </div>
      ) : (
